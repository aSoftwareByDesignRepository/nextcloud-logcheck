 ...
 *
 * Drops every table the logcheck app has ever created, migration rows, and app config.
 *
 ...
 *     php scripts/check-nextcloud-db-standards.php sync-uninstall --app=logcheck
 *
...
final class UninstallDropTables implements IRepairStep
{
	...
	{
		return 'Drop logcheck tables and install metadata on uninstall';
	}
	...
	public function run(IOutput $output): void
	{
		...
		$output->info(
			'logcheck: preserving data on disable. '
	...
	private function dropAllTablesAndMetadata(IOutput $output): void
	{
		...
		$output->info(sprintf(
			'logcheck: dropped %d of %d table(s); removed %d migration row(s), app config, and upgrade-backup snapshots.',
			$dropped,
			count(self::TABLES),
			$migrationsRemoved,
		));
	...
	private function purgeUpgradeBackupSnapshots(IOutput $output): void
	{
		...
		$node->delete();
		$output->info('logcheck: removed upgrade-backup snapshots from app data.');
	}
}
