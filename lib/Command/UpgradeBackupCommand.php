...
use OCA\LogCheck\Service\UpgradeBackupIntegrity;
...
<info>create</info> — manual snapshot before risky changes:
  <comment>occ logcheck:upgrade-backup create --reason="before manual fix"</comment>

<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ logcheck:upgrade-backup restore --latest --force</comment>
  <comment>occ logcheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ logcheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ logcheck:upgrade-backup restore --latest --force',
				'  occ logcheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ logcheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
