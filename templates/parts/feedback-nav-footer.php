 ...
 * @var \OCP\IL10N $l
 * @var \OCA\LogCheck\Support\AppFeedbackLinks $appFeedbackLinks optional; constructed when omitted
...
if ($version === '' && class_exists(\OCP\Server::class)) {
	try {
		...
		$resolved = trim((string)$appManager->getAppVersion('logcheck'));
		if ($resolved !== '') {
			$version = $resolved;
		}
	} catch (\Throwable) {
