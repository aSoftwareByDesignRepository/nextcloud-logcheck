/**
 * HealthCheck mobile navigation: in-page Menu + slide-in drawer.
...
(function () {
	...
	function init() {
		var toggle = document.querySelector('[data-lck-nav-toggle]');
		var nav = document.getElementById('app-navigation');
		if (!toggle || !nav) {
			return;
		}

		document.body.classList.add('lck-has-app-nav');
		...
		var openLabel =
			...
			translate('Toggle navigation menu', 'Toggle navigation menu');
		var closeLabel =
		...
		function setOpen(open) {
			nav.classList.toggle('lck-nav--open', open);
			toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
			toggle.setAttribute('aria-label', open ? closeLabel : openLabel);
			document.body.classList.toggle('lck-nav-open', open);
			document.body.classList.remove('snapjs-left');
			...
			if (open) {
				...
				trapHandler = function (event) {
					if (!nav.classList.contains('lck-nav--open') || event.key !== 'Tab') {
						return;
					}
				...
				pointerCloseHandler = function (event) {
					if (!nav.classList.contains('lck-nav--open')) {
						return;
					}
		...
		toggle.addEventListener('click', function (event) {
			...
			setOpen(!nav.classList.contains('lck-nav--open'));
		});

		document.addEventListener('keydown', function (event) {
			if (event.key === 'Escape' && nav.classList.contains('lck-nav--open')) {
				setOpen(false);
				toggle.focus();
			}
		});
