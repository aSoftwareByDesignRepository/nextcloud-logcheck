...
function boot(extra) {
	...
	nodes.set('lck-app-feedback-config', cfgEl);
	...
	nodes.set('lck-feedback-problem', problem);
	nodes.set('lck-feedback-idea', idea);

	const trigger = {
		className: 'lck-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'lck-feedback-menu';
	const menu = {
		id: 'lck-feedback-menu',
		hidden: true,
		contains() {
			return false;
		},
	...
	nodes.set('lck-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.lck-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'lck-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/logcheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/logcheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/logcheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/logcheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/logcheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (logcheck)\n');
}
