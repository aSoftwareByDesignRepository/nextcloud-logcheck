...
We support the latest published LogCheck release on currently supported Nextcloud majors listed in `appinfo/info.xml`.
...
- Older copies next to the live file (Nextcloud `.1`…`.50` and LogCheck start-fresh archives) can be listed and opened by **basename id only** — never a client-supplied path. Alert watching always stays on the live systemconfig file.
...
LogCheck is supported on **one** Nextcloud server (or a cluster that shares the **same** log file). Multiple application servers each with their own local log file are **not supported** — digests would miss or duplicate lines.
